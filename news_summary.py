...
def extract_content_with_jina(url):
    """
    Extract content dari URL menggunakan requests + BeautifulSoup (fallback jika Jina error).
    Cepat, ringan, dan tidak bergantung pada layanan eksternal.
    """
    print(f"📖 Extracting content with BeautifulSoup: {url}")
    ...
    try:
        ...
        # Ambil paragraf (5-6 paragraf pertama untuk konteks AI)
        paragraphs = soup.find_all('p')
        content_text = ""
        count = 0
        for p in paragraphs:
            text = p.get_text(strip=True)
            if len(text) > 20:  # Abaikan paragraf terlalu pendek
                content_text += text + "\n"
                count += 1
                if count >= 6:
                    break
        ...
        if not content_text and title_text:
            # Fallback: ambil semua text body
            body = soup.find('body')
            if body:
        ...
        return full_content[:15000]  # Limit untuk input AI
        
    except Exception as e:
        print(f"❌ Error extracting content: {e}")
        # Fallback ke Jina AI jika BeautifulSoup gagal
        try:
            print(f"🔄 Falling back to Jina AI Reader...")
            jina_url = f"https://r.jina.ai/{url}"
            response = requests.get(jina_url, timeout=20)
            if response.status_code == 200:
                print(f"✅ Jina AI fallback successful")
...
def main():
    ...
    for i, article in enumerate(articles_to_process, 1):
        ...
        # Extract content dengan Jina AI (cepat, tanpa browser)
